...
def place_ships(player, rounds, board_ships_p, ships_placed):
    ...
    for length in range(len(SHIPS) - 1, 0, -1):
        for i in range(SHIPS[length]):
            # Print message to the player to place a ship of the given length
            print("Place a ship of size " +  + "!")
            ...
            for j in range(length):
                if rotate:  # will rotate with the given numbers
                    # Add the coordinates to the new ship
            ...
            # Append the new ship to the list of ships placed
            ships_placed[player].
